...
class HertzAPIService: ObservableObject {
    static let shared = HertzAPIService()
    ...
    private var baseURL: String {
        "http://\(HertzSettings.shared.ipAddress)/YamahaExtendedControl/v1"
    }
    ...
    func fetchStatus() {
        ...
        guard !HertzSettings.shared.ipAddress.isEmpty else {
    ...
    func reboot() {
        guard let url = URL(string: "\(baseURL)/system/requestSystemReboot") else { return }
        session.dataTask(with: url) { _, _, _ in }.resume()
    }
    ...
    func fetchFuncStatus() {
        ...
        session.dataTask(with: url) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async {
                ...
                self.availableInputs = HertzAPIService.allSources.map { $0.value }
            }
        }.resume()
    }
    ...
    func fetchNetRadioListFull(completion: @escaping (Result<NetRadioLayer, Error>) -> Void) {
        fetchNetRadioPagedFrom(index: 0, accumulated: [], completion: completion)
    }

    private func fetchNetRadioPagedFrom(index: Int, accumulated: [NetRadioListItem],
                                         retries: Int = 8,
                                         completion: @escaping (Result<NetRadioLayer, Error>) -> Void) {
        let size = 8
        guard let url = URL(string: "\(baseURL)/netusb/getListInfo?input=net_radio&index=\(index)&size=\(size)&lang=en") else {
        ...
        session.dataTask(with: url) { [weak self] data, _, error in
            ...
            // Network error or no data — retry
            guard let data,
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                    self.fetchNetRadioPagedFrom(index: index, accumulated: accumulated, retries: retries - 1, completion: completion)
                }
                return
            }

            let code = json["response_code"] as? Int ?? -1
            let status = json["menu_status"] as? String ?? ""

            // Receiver busy or not ready — retry
            if code != 0 || status == "busy" {
                guard retries > 0 else {
                    DispatchQueue.main.async { completion(.failure(URLError(.badServerResponse))) }
                    return
                }
                let delay: Double = code == 4 ? 1.2 : 0.8
            ...
            let name     = json["menu_name"]  as? String ?? "Net Radio"
            let layer    = json["menu_layer"] as? Int ?? 0
            let maxLine  = json["max_line"]   as? Int ?? 0
            let raw      = json["list_info"]  as? [[String: Any]] ?? []
            let newItems = raw.enumerated().map { i, d in
                NetRadioListItem(id: index + i,
                                 text: d["text"] as? String ?? "",
                                 attribute: d["attribute"] as? Int ?? 0)
            }
            let all = accumulated + newItems
            let nextIndex = index + size
            if nextIndex < maxLine && !newItems.isEmpty {
                self.fetchNetRadioPagedFrom(index: nextIndex, accumulated: all, retries: retries, completion: completion)
            } else {
                DispatchQueue.main.async {
                    completion(.success(NetRadioLayer(name: name, layer: layer, items: all)))
                }
            }
        }.resume()
    }

    func resetNetRadioToRoot(completion: @escaping () -> Void) {
        guard let url = URL(string: "\(baseURL)/netusb/getListInfo?input=net_radio&index=0&size=1&lang=en") else {
            completion(); return
        }
        session.dataTask(with: url) { [weak self] data, _, _ in
            guard let self,
                  let data,
                  let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
                  let layer = json["menu_layer"] as? Int, layer > 0 else {
                DispatchQueue.main.async { completion() }
                return
            }
            // Navigate back one level and retry
            guard let retUrl = URL(string: "\(self.baseURL)/netusb/setListControl?list_id=\(layer)&type=return&zone=main") else {
                DispatchQueue.main.async { completion() }
                return
            }
            self.session.dataTask(with: retUrl) { [weak self] _, _, _ in
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
                    self?.resetNetRadioToRoot(completion: completion)
                }
            }.resume()
        }.resume()
    }

    func selectNetRadioItem(layer: Int, index: Int,
                            completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "\(baseURL)/netusb/setListControl?list_id=\(layer)&type=select&index=\(index)&zone=main") else { return }
        session.dataTask(with: url) { data, _, _ in
            let ok = (try? JSONSerialization.jsonObject(with: data ?? Data()) as? [String: Any])?["response_code"] as? Int == 0
            DispatchQueue.main.async { completion(ok) }
        }.resume()
    }

    func playNetRadioItem(layer: Int, index: Int,
                          completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "\(baseURL)/netusb/setListControl?list_id=\(layer)&type=play&index=\(index)&zone=main") else { return }
        session.dataTask(with: url) { data, _, _ in
            let ok = (try? JSONSerialization.jsonObject(with: data ?? Data()) as? [String: Any])?["response_code"] as? Int == 0
            DispatchQueue.main.async { completion(ok) }
        }.resume()
    ...
    func searchNetRadio(layer: Int, index: Int, query: String,
                        retries: Int = 5,
                        completion: @escaping (Bool) -> Void) {
        guard let enc = query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let url = URL(string: "\(baseURL)/netusb/setSearchString?list_id=\(layer)&string=\(enc)&index=\(index)&zone=main") else { return }
        session.dataTask(with: url) { [weak self] data, _, _ in
            let code = (try? JSONSerialization.jsonObject(with: data ?? Data()) as? [String: Any])?["response_code"] as? Int ?? -1
            if code == 0 {
                DispatchQueue.main.async { completion(true) }
            } else if retries > 0 {
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                    self?.searchNetRadio(layer: layer, index: index, query: query, retries: retries - 1, completion: completion)
                }
            } else {
    ...
    func storePreset(_ num: Int) {
        guard let url = URL(string: "\(baseURL)/netusb/storePreset?zone=main&num=\(num)") else { return }
        ...
        session.dataTask(with: url) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async {
                ...
                if let artPath = json["albumart_url"] as? String,
                   ...
                   artId > 0,
                   !artPath.isEmpty {
                    let newURL = "http://\(HertzSettings.shared.ipAddress)\(artPath)"
...
struct NetRadioListItem: Identifiable {
    let id: Int          // index in list
    let text: String
    let attribute: Int   // 1=playable, 2=directory, 8=search, 10=search+dir
    var isDirectory: Bool { attribute & 2 != 0 && attribute & 8 == 0 }
    var isPlayable:  Bool { attribute & 2 == 0 && attribute & 8 == 0 }
    var isSearch:    Bool { attribute & 8 != 0 }
}

struct NetRadioLayer {
    let name: String
    let layer: Int
    let items: [NetRadioListItem]
}
